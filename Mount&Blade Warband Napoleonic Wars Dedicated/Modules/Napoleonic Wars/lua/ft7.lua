...
function checkDuelValid(player1, player2)
    if(player2 == nil) then
        if(ft7_players[player1] ~= nil) then
            sendServerMessage(player1, "You are in ft7 mode, you can't duel another bot!")
            return false
        else
            return true
        end
    ...
    elseif(ft7_players[player1] == nil and ft7_players[player2] ~= nil) then
        ...
        return false
    elseif(ft7_players[player1] ~= nil and ft7_players[player2] == nil) then
        sendServerMessage(player1, "You are in ft7 mode, you can't duel with non-ft7 players!")
        return false
    else
...
function playerEnterFt7(player)
    if(ft7_players[player] == nil) then
        ...
        sendServerMessage(player, "You are ready for a First to Seven duel match, find another FT7 player to duel.")
    else
        sendServerMessage(player, "You already entered First to seven mode!")
    end
end
...
function listFt7(player)
    sendServerMessage(player, "Players who are in FT7 mode:")
    for ft7_player_id, ft7_player in pairs(ft7_players) do
        ...
        if(ft7_player["opponent"] == nil) then
            ...
            sendServerMessage(player, ft7_player_name)
        end
    end
    sendServerMessage(player, "Ongoing FT7 Duel matches:")
    for ft7_player_id, ft7_player in pairs(ft7_players) do
        if(ft7_player["visited"] == false) then
            ...
            sendServerMessage(player, player1_name .. " " .. ft7_players[ft7_player_id]["score"] .. "-" .. ft7_players[opponent]["score"] .. " " .. player2_name )
        end
    end
end
